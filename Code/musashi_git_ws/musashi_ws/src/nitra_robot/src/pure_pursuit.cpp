...
PurePursuit::PurePursuit(ros::NodeHandle nh, ros::NodeHandle private_nh, double sampling_time)
{
    robot_state_client = nh.serviceClient<robot_state_manager::RobotState>("robot_state_manager_service");
...
void PurePursuit::setTrajectory(sdv_msgs::Trajectory &traj)
{
...
void PurePursuit::control(const Eigen::Vector3d &rear_axis_pose)
{
    ...
    {
        Eigen::Vector3d first_error = first_pos - rear_axis_pose;
        check_heading = checkOrientation(first_error);
        ...
        {
            ROS_INFO("ROTATE");
            rotate(rear_axis_pose);        
        }
        
    }
    else if (rotation_mode == false)
    {
        // calculate error    
        ...
        {
            stopMotion(rear_axis_pose);
            ros::Duration(1).sleep();
            robot_state_srv.request.robot_state_request = "Reached_goal";
...
// void PurePursuit::pubGoalStatus(void)       // Musashi AI 
// {
...
void PurePursuit::rotate(const Eigen::Vector3d &rear_axis_pose)
{

    Eigen::Vector2d input(0.0, 0.5);
...
bool PurePursuit::checkOrientation(const Eigen::Vector3d &error)
{
    if(std::abs(error(2))>6.0)
    {
        if ((std::abs(error(2)) - MathConstants::TWOPI) < FIRST_HEADING_THRESHOLD)
        {
            return true;
        }
    }
    if (std::abs(error(2)) < FIRST_HEADING_THRESHOLD)
    {
        return true;
    }
    return false;
