...
void PurePursuit::includeTIME()
{
    std::ofstream export_data;
    char name[30];

    export_data.open("time.csv", std::ios::out|std::ios::app);

    time_t now = time(0);
    strftime(name, sizeof(name), "%Y%m%d_%H%M", localtime(&now));

    export_data << name << "," << run_time_;
