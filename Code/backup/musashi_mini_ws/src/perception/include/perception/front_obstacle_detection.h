...
public:
    FrontObstacleDetection(ros::NodeHandle nh, ros::NodeHandle private_nh)
    {
        ...
        nh_.param("map_width", map_width_, 1.5);
        ...
        map_metadata_msg_.origin.position.z = (transform_odom_base_ * transform_base_lidar_).getOrigin().getZ(); // ground
...
*/
        ...
        min_x_ = 0.0;
        max_x_ = map_width_;
        min_y_ = (-map_height_ / 2) - transform_base_lidar_.getOrigin().getY();
        max_y_ = (map_height_ / 2) - transform_base_lidar_.getOrigin().getY();
        ...
        ;
        Eigen::Vector3f rotation(0, 0, 0);  //// EDITED original (0,0,-yaw) ~~BUG of cropping when yaw doesnt initialize as 0.

        boxFilter_.setRotation(rotation);
        // boxFilter_.setTranslation(translation);
        boxFilter_.setMin(Eigen::Vector4f(min_x_, min_y_, 0.0f, 1.0f));
        boxFilter_.setMax(Eigen::Vector4f(max_x_, max_y_, 0.1f, 1.0f));

        std::cout << "tf x :" << -transform_base_lidar_.getOrigin().getX() << std::endl;
        std::cout << "tf y :" << -transform_base_lidar_.getOrigin().getY() << std::endl;
        std::cout << "tf yaw :" << yaw << std::endl;
