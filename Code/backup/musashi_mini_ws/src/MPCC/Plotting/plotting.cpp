...
void Plotting::plotRun(const std::list<MPCReturn> &log, const TrackPos &track_xy) const
{
    ...
    */
        std::vector<double> plot_x_i;
    std::vector<double> plot_y_i;

    std::vector<double> plot_x_o;
    std::vector<double> plot_y_o;

    double tolerence = 0.0;
    double a = 0.0;
    double b = 1.0 - tolerence;
 

    while(a<=4.25 - tolerence)
    {
        plot_x_i.push_back(a);
        plot_y_i.push_back(b);
        a = a + 0.1;
    }

    while(b>=-4.0)
    {
        plot_x_i.push_back(4.25);
        plot_y_i.push_back(b);
        b = b - 0.1;
    }
   ...
   b = -1.0;
    ...
    while(a<=3.75 - tolerence)
    {
        plot_x_o.push_back(a);
        plot_y_o.push_back(b);
        a = a + 0.1;
    }

    while(b>=-4.0)
    {
        plot_x_o.push_back(3.75);
        plot_y_o.push_back(b);
        b = b - 0.1;
    }

    plt::figure(1);
    plt::scatter(plot_xc,plot_yc);
    plt::plot(plot_x_i,plot_y_i,"k-");
    plt::plot(plot_x_o,plot_y_o,"k-");
