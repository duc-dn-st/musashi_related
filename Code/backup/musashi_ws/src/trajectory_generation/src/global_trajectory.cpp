...
#include <std_msgs/Float32MultiArray.h>     // For test route selection
...
void Global_traj_class::setRoute(double route_array[], int route_array_length, double robot_current_pos[])
{

    ROS_INFO("Set Route");

    int route_points_count = route_array_length/2;
    ...
    //--------------------------------------------------------------------------------------------------------------------------
    // For test route selection
    // boost::shared_ptr<std_msgs::Float32MultiArray const> test_route_array;
    // test_route_array = ros::topic::waitForMessage<std_msgs::Float32MultiArray>("test_send_points", nh_, ros::Duration(10.0));

    // if (test_route_array != nullptr){

    default_way_points.resize(route_points_count, 2);

    unsigned int ii = 0;
    for (unsigned int i = 0; i < route_array_length; i+=2)
    {
        default_way_points.row(ii).col(0) << route_array[i];
        default_way_points.row(ii).col(1) << route_array[i+1];
        ii++;
    }
    ...
    // wait for RVIZ
    // } else {
    //     std::cout << "*** Route not selected ***" << std::endl;
    //     std::cout << "*** Selecting default route ***" << std::endl;
    //     std::cout << "Route points count : " << route_points_count + 1 << "\n" << std::endl;
    ...
    //                 default_way_points.resize(route_points_count, route_points_count);
                    
    //                 for (int i = 0; i < route_points_count; i++) {
    //                     default_way_points(i,0) = route_array[2*i];
    //                     default_way_points(i,1) = route_array[2*i+1];
    //                 }
    ...
    way_points.row(0) << robot_current_pos[0], robot_current_pos[1];
